...
export class ValidationFactory {
    ...
    static async validateDocument(document, config = {}) {
        const { preset = 'standard', elementValidation = true, documentValidation = true, documentOptions } = config;
...
export const SVGValidator = {
     ...
     */
    quick: (document) => ValidationFactory.quickValidate(document),
    /**
     * Standard validation with balanced checks
     */
    standard: (document) => ValidationFactory.validateDocument(document, { preset: 'standard' }),
    /**
     * Strict validation with all checks enabled
     */
    strict: (document) => ValidationFactory.validateDocument(document, { preset: 'strict' }),
    /**
     * Performance-focused validation
     */
    performance: (document) => ValidationFactory.validateDocument(document, { preset: 'performance' }),
    /**
     * Accessibility-focused validation
     */
    accessibility: (document) => ValidationFactory.validateDocument(document, { preset: 'accessibility' }),
    /**
     * Validation with automatic fixes applied
     */
    withAutoFix: (document, preset) => ValidationFactory.validateWithAutoFix(document, preset)
};
